...
> 161 Python 文件 · 24,000+ 行代码 · 完全原创 · 零外部框架依赖
...
```
cogu/
├── core/           ← 核心引擎 (ReAct Agent, 4级护栏, 推理链, 二级规划, Heartbeat心跳)
├── memory/         ← 超级记忆 (FTS5 + 向量 + 图 + 分级压缩 + Dream + TaskMemory + CodingMemory)
├── evolution/      ← 自进化 (技能/Prompt/记忆进化, 适应度评估, Trajectory轨迹)
├── debate/         ← 思辨团队 (6种专家, 5种协调, PES引擎, 超图编排)
├── tools/          ← 工具系统 (可插拔注册, MCP协议, 原子进化)
├── skills/         ← 技能系统 (Markdown规范, 自动发现, 行为克隆)
├── mcp/            ← MCP全栈 + A2A Agent-to-Agent协议 (Schema归一化, 安全认证, 多传输)
├── search/         ← DeepSearch 知识增强深度搜索 (查询规划→多步搜索→来源溯源→报告生成)
├── studio/         ← AgentStudio 可视化工作流引擎 (DSL编译, 节点图, Mermaid导出)
...
├── compression/    ← 上下文压缩 + ContextEngine处理器链
...
### P0 — openJiuwen 融合 (v1.1.0 新增)

| 模块 | 功能 | 灵感来源 |
|------|------|---------|
| `core/heartbeat.py` | 周期性Agent唤醒 (活跃时段 + busy backoff + HEARTBEAT.md驱动) | jiuwenswarm GatewayHeartbeatService |
| `memory/task_memory.py` | 任务级经验检索与存储 (JSON持久化 + 多维检索) | jiuwenswarm TaskMemoryService |
| `memory/coding_memory.py` | 项目级代码记忆 (.md文件 + 按项目分目录 + 搜索) | jiuwenswarm CodingMemoryRail |
| `evolution/trajectory.py` | 完整执行轨迹数据模型 (LLM/Tool调用 + 奖励 + 注册表) | openjiuwen agent_evolving/trajectory |
| `core/rituals.py` 增强 | Cron持久化 + 多渠道推送 (JSON存储 + ChannelTarget) | jiuwenswarm gateway/cron |

### P0 — A2A + DeepSearch + ContextEngine + AgentStudio (v1.2.0 新增)

| 模块 | 功能 | 灵感来源 |
|------|------|---------|
| `mcp/a2a_adapter.py` | A2A Agent-to-Agent 协议 (Agent Card发现 + 任务通信 + SSE流式) | openJiuwen agent-protocol/A2A |
| `search/deep_search.py` | 知识增强深度搜索 (查询规划→多步搜索→来源溯源→报告生成) | openJiuwen DeepSearch |
| `compression/context_engine.py` | 插件式上下文处理器链 (滑动窗口+Token预算+摘要+去重) | openJiuwen ContextEngine |
| `studio/workflow_engine.py` | 可视化工作流引擎 (DSL编译+节点图+Mermaid导出+执行引擎) | openJiuwen AgentStudio |
...
| **openJiuwen (v1.1.0 新增)** | **Heartbeat心跳, TaskMemory任务记忆, CodingMemory代码记忆, Trajectory轨迹模型, CronJob持久化调度** |
| **openJiuwen (v1.2.0 新增)** | **A2A Agent-to-Agent协议, DeepSearch深度搜索, ContextEngine处理器链, AgentStudio可视化工作流** |
...
| CogAgent (清华) | GUI动作空间 |
| JiuwenCLAW (华为) | Runner单例, 会话管理, Heartbeat心跳, TaskMemory, CronJob |
