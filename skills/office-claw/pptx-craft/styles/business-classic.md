...
- 字体：使用 CSS 字体栈 `WenYuan Sans SC`, `Noto Sans SC`, `sans-serif`；浏览器按可用字体自动降级
- 字号三种以内
- 决不允许出现动画
- 符合公司视觉规范
- 关键内容标红加粗
...
- **元素**：
  ...
  - 增长/对比：14px，灰色（辅助数据说明）
  - 简要说明：14px，黑色/80%透明度（一行概括功能或成果）
- **技术实现**：
...
chart.setOption({
  xAxis: {
    axisLabel: { fontSize: 14, color: '#2B2D42' },  // ✅ 深色文字
    // axisLabel: { color: '#a6a6a6' },  // ❌ 禁止浅灰色
  },
  yAxis: {
    axisLabel: { fontSize: 14, color: '#2B2D42' },
...
- 内容区有效区域尺寸为 **1220×660px**（1280×720 页面减去左右各 30px 边距）
